...
calculate_network_stats_all_frames <- function(edgelist, seq, state) {
    ...
    ratio <- SASA_files[which(SASA_files$label == seqname), ratio_col]
    if (length(ratio) == 0) {
        ratio <- 100
    }
...
# Function to generate dipeptide sequences
generate_peptides <- function(length) {
    all_peptides <- c()
    for (pos in 1:length) {
        if (pos == 1) {
            for (r2 in residues) {
                all_peptides <- c(all_peptides, paste("C", r2, sep = "_"))
            }
        } else if (pos == 2) {
            for (r1 in residues) {
                all_peptides <- c(all_peptides, paste(r1, "C", sep = "_"))
            }
        }
    }
    return(all_peptides)
...
SASA_files <- read.table(
    "/dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/AP_analysis/Dipeptide/SASA_score_M21/SASA_result_with_common_mon_ini.txt",
    header = FALSE,
    col.names = c("Amino1", "Amino2", "ratio_mon_fin", "ratio_dim_fin")
)
SASA_files$label <- paste0(SASA_files$Amino1, SASA_files$Amino2)
...
for (pep_type in names(peptide_types)) {
    ...
    for (state in states) {
        ...
        rm(monomer_edgelist)
        # Load overall edgelist files
        edgelist_filename <- if(state == "monomer") {
            file.path(main_dir, paste0("dipeptide_edgelist_", state, ".rda"))
        } else {
            file.path(main_dir, paste0("dipeptide_edgelist_", state, "_single_node.rda"))
        }
        load(edgelist_filename)
    ...
    # Save results
    save(final_results, file = file.path(main_dir, paste0(pep_type, "_network_stats",
