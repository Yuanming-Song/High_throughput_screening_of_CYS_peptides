...
for sequence in "${seqs[@]}"; do
    # Extract the first and second amino acids
    amino1=${sequence:0:1}
    amino2=${sequence:1:1}
    ...
    if ( (is_in_array "$amino1" "${positive_charged[@]}" && is_in_array "$amino2" "${positive_charged[@]}") ||
        (is_in_array "$amino1" "${negative_charged[@]}" && is_in_array "$amino2" "${negative_charged[@]}") ||
        ( (is_in_array "$amino1" "${positive_charged[@]}" || is_in_array "$amino1" "${negative_charged[@]}") &&
            ! (is_in_array "$amino2" "${positive_charged[@]}" || is_in_array "$amino2" "${negative_charged[@]}")) ||
        ( (is_in_array "$amino2" "${positive_charged[@]}" || is_in_array "$amino2" "${negative_charged[@]}") &&
            ! (is_in_array "$amino1" "${positive_charged[@]}" || is_in_array "$amino1" "${negative_charged[@]}"))); then
        gpu_type="A100:1"
    else
        gpu_type="A30:1"
    fi
    ...
    new_job_id=$(
        sbatch --parsable \
            ...
            --cpus-per-task=8 \
            --time=5:00:00 \
            --out=out/C_${amino2}.out \
            --gres=gpu:${gpu_type} \
            --wrap="bash /dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/Dipeptide/SIRAH/Dipeptide_SIRAH_run.sh ${amino1} ${amino2} "
    )
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "Failed to submit job for C_${amino2}_mon"
    fi
