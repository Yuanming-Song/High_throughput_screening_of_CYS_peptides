...
for Cindex in 1 2 3; do
    ...
    outfile="SASA_result_Tripeptide_C${Cindex}_mon.dat"
    missing_vdw_log="missing_vdw_mon_C$Cindex.log"
    rm $outfile
    if [[ "$Cindex" -eq 3 ]]; then

        maindir="/share/crsp/lab/dtobias/yuanmis1/mrsec/ML-MD-Peptide/MARTINI22/Tripeptide_mon_C${Cindex}_redo/" # Replace with your actual main directory path
    else
        maindir="/share/crsp/lab/dtobias/yuanmis1/mrsec/ML-MD-Peptide/MARTINI22/Tripeptide_mon_C${Cindex}/" # Replace with your actual main directory path
    ...
    for amino1 in "${amino_acids[@]}"; do
        ...
        for amino2 in "${amino_acids[@]}"; do
            ...
            for amino3 in "${amino_acids[@]}"; do
                ...
                if [ ! -f "$xtc_file" ]; then
                    echo "no xtc file ${amino1}_${amino2}_${amino3}"
                    continue
                fi
                ...
                if [ ! -f "$log_file" ]; then
                    echo "no log file ${amino1}_${amino2}_${amino3}"
                    continue
                fi
                ...
                output_xvg="out/${amino1}_${amino2}_${amino3}_mon.xvg"
                echo 1 | gmx sasa -f "$xtc_file" -s "$gro_file" -o "$output_xvg" >log/SASA_${amino1}_${amino2}_${amino3}_mon.log 2>&1

                output_xvgi="out/${amino1}_${amino2}_${amino3}_mon_ini.xvg"
                echo 1 | gmx sasa -f "$gro_file" -s "$gro_file" -o "$output_xvgi" >log/SASA_${amino1}_${amino2}_${amino3}_mon_ini.log 2>&1

                # Check if the log file contains the warning
                if grep -q "WARNING: could not" "log/SASA_${amino1}_${amino2}_${amino3}_mon_ini.log"; then
                    echo "Missing VDW radii in ${amino1}_${amino2}_${amino3}: Skipping this loop." >>$missing_vdw_log
                    continue
                fi
