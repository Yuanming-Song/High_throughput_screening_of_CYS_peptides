...
for amino1 in "${amino_acids[@]}"; do
        ...
        sbatch --job-name="${amino1}_C" \
               ...
               --cpus-per-task=40 \
               --time=24:00:00 \
               --out=out/${amino1}_C.out \
               --wrap="bash /dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/dipeptide/SetupScript/Cys_stapled_dipeptide_MARTINI_run.sh ${amino1} C  "
done
