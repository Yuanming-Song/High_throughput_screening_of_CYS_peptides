...
binwidth<-1
outdir<-"/dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/Edgelist/Dipeptide"
...
# Set main directory
maindir <- "/dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/Dipeptide/MARTINI21"
...
for (pos in positions) {
  if (pos == 1) {
    sequences <- sapply(residues, function(x) paste0("C_", x))
  } else {
    sequences <- sapply(residues, function(x) paste0(x, "_C"))
  }
  ...
  for (seq in sequences) {
    for (state in statelist) {
      ...
      # Save edgelist to appropriate directory
      if (exists("edgelist")) {
        if (state == "dis") {
    ...
    save(dimer_sizehis,file=file.path(csizedir,paste0("dimer_cdist_dipeptide",if (single_node) "_single_node" else "",".rda")))
    save(monomer_sizehis,file=file.path(csizedir,"monomer_cdist_dipeptide.rda"))
  }
} 