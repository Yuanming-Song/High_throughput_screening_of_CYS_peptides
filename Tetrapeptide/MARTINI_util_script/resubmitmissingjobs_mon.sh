...
# Define allowed cysteine positions (1-based indexing)
cindexlist=( 3 )

# Define list of sequences to avoid
avoid_sequences=("S_F_C_R") # Example: ("A_C_G_H" "F_C_I_K")

# Define the list of amino acids (excluding Cys for non-C positions)
amino_acids=( "C" "A" "F" "G" "H" "I" "K" "L" "M" "N" "P" "Q" "R" "S" "T" "V" "W" "Y" "E" "D")
    ...
    for aa2 in "${amino_acids[@]}"; do
        for aa3 in "${amino_acids[@]}"; do
            for aa4 in "${amino_acids[@]}"; do
                ...
                # Check if cindex is in the allowed positions
                if [[ ! " ${cindexlist[@]} " =~ " ${cindex} " ]]; then
                    continue
                fi
                ...
                # Check if jobname is in avoid_sequences
                if [[ " ${avoid_sequences[@]} " =~ " ${jobname} " ]]; then
                    continue
                fi
