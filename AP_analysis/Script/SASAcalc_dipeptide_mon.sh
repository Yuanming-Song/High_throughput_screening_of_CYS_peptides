...
/dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/AP_analysis/Script/vdwradiigenerator.sh
for Cindex in 1 2; do
    outfile="SASA_result_Dipeptide_C${Cindex}_mon.dat"
    missing_vdw_log="missing_vdw.log"
    rm $outfile
    ...
    maindir="/share/crsp/lab//dtobias/yuanmis1/mrsec/ML-MD-Peptide/MARTINI21/Dipeptide_mon_C${Cindex}/" # Replace with your actual main directory path

    # Initialize the log file to record missing VDW radii
    echo "Missing VDW Radii Log" >$missing_vdw_log
    echo "=====================" >>$missing_vdw_log
    #run vdwradii.dat file generator
    ...
    for amino1 in "${amino_acids[@]}"; do
        ...
        for amino2 in "${amino_acids[@]}"; do
            ...
            xtc_file="${workdir}/${amino1}_${amino2}_CG_md.xtc"
            if [ ! -f "$xtc_file" ]; then
                echo "no xtc ${amino1}_${amino2}"
                continue
            fi

            # Check if the .log file exists
            log_file="${workdir}/${amino1}_${amino2}_CG_md.log"
            if [ ! -f "$log_file" ]; then
                echo "no log file ${amino1}_${amino2}"
                continue
            fi
            ...
            echo 1 | gmx sasa -f "$xtc_file" -s "$gro_file" -o "$output_xvg" >log/SASA_${amino1}_${amino2}_mon.log 2>&1
            ...
            if grep -q "WARNING: could not" "log/SASA_${amino1}_${amino2}_mon_ini.log"; then
                echo "Missing VDW radii in ${amino1}_${amino2}: Skipping this loop." >>$missing_vdw_log
                continue
            fi
