...
classify_sequence <- function(a1, a2) {
  ...
  # For dipeptides, we'll classify based on the non-C amino acid
  non_c_aa <- amino_acids[amino_acids != "C"][1]
  
  if (non_c_aa %in% hydrophobic) {
    return("Hydrophobic Side Chain")
  } else if (non_c_aa %in% polar_uncharged) {
    return("Polar Uncharged Side Chain")
  } else if (non_c_aa %in% c(positive_charged, negative_charged)) {
    return("Charged Side Chain")
  } else if (non_c_aa %in% aromatic) {
    return("Aromatic")
  }
...
edge_data <- combined_df_ddedge_dipep[combined_df_ddedge_dipep$stat_name == "edges" & 
                                     combined_df_ddedge_dipep$frame == max(combined_df_ddedge_dipep$frame), ]

# Process the AP score data (using ratio column)
ap_data <- combined_df_ddedge_dipep[combined_df_ddedge_dipep$frame == max(combined_df_ddedge_dipep$frame), ]
...
edge_correlation_plot <- create_correlation_plot(
  ...
  "Edge Correlation in Dipeptides",
  300
)
...
ap_correlation_plot <- create_correlation_plot(
  ...
  "AP Score Correlation in Dipeptides"
)
...
if (exists("save_plots") && save_plots) {
  ggsave(paste0(pltsavedir, "/Edge_correlation_by_C_position_dipep.png"), 
  ...
  ggsave(paste0(pltsavedir, "/AP_correlation_by_C_position_dipep.png"), 
...
edge_latex_table <- c(edge_latex_table,
                     "        \\hline",
                     "    \\end{tabular}",
                     "    \\caption{Top 10 dipeptide sequences ranked by the highest difference in network edges (dEdge) between dimeric and monomeric forms. logP$^\\prime$ is the normalized Wimley-White hydrophobicity scale calculated as (logP - logP$_{min}$)/(logP$_{max}$ - logP$_{min}$), where logP$_{min}$ and logP$_{max}$ are the theoretical minimum and maximum values possible for a dipeptide.}",
                     "    \\label{tab:di_top10_dedge}",
                     "\\end{table}")
...
edge_by_category_latex_table <- c(edge_by_category_latex_table,
                     "        \\hline",
                     "    \\end{tabular}",
                     "    \\caption{Top dipeptide sequence with the highest dEdge from each sidechain chemical category. logP$^\\prime$ is the normalized Wimley-White hydrophobicity scale calculated as (logP - logP$_{min}$)/(logP$_{max}$ - logP$_{min}$), where logP$_{min}$ and logP$_{max}$ are the theoretical minimum and maximum values possible for a dipeptide.}",
                     "    \\label{tab:di_top_bycat_dedge}",
                     "\\end{table}")
...
ap_latex_table <- c(ap_latex_table,
                   "        \\hline",
                   "    \\end{tabular}",
                   "    \\caption{Top 10 dipeptide sequences ranked by the highest difference in Aggregation Propensity (dAP) between dimeric and monomeric states. logP$^\\prime$ is the normalized Wimley-White hydrophobicity scale calculated as (logP - logP$_{min}$)/(logP$_{max}$ - logP$_{min}$), where logP$_{min}$ and logP$_{max}$ are the theoretical minimum and maximum values possible for a dipeptide.}",
                   "    \\label{tab:di_top10_dap}",
                   "\\end{table}")
...
ap_by_category_latex_table <- c(ap_by_category_latex_table,
                   "        \\hline",
                   "    \\end{tabular}",
                   "    \\caption{Top dipeptide sequence with the highest dAP from each sidechain chemical category. logP$^\\prime$ is the normalized Wimley-White hydrophobicity scale calculated as (logP - logP$_{min}$)/(logP$_{max}$ - logP$_{min}$), where logP$_{min}$ and logP$_{max}$ are the theoretical minimum and maximum values possible for a dipeptide.}",
                   "    \\label{tab:di_top_bycat_dap}",
                   "\\end{table}")

# Save the LaTeX tables if save_plots is true
if (exists("save_plots") && save_plots) {
  writeLines(edge_latex_table,
            paste0(pltsavedir, "/top_10_dedge_dipep.tex"))
  writeLines(edge_by_category_latex_table,
             paste0(pltsavedir, "/top_by_category_dedge_dipep.tex"))
  writeLines(ap_latex_table,
            paste0(pltsavedir, "/top_10_dap_dipep.tex"))
  writeLines(ap_by_category_latex_table,
             paste0(pltsavedir, "/top_by_category_dap_dipep.tex"))
} 