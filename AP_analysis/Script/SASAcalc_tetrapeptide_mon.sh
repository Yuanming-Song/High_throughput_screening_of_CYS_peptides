...
mkdir -p out log SubData

outfile="SubData/SASA_result_Tetrapeptide_C${Cindex}_${first_res}_mon.dat"
missing_vdw_log="missing_vdw_mon_C${Cindex}_${first_res}.log"
rm -f $outfile

maindir="/dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/Tetrapeptide/MARTINI22/Tetrapeptide_mon_C${Cindex}/"
    ...
    for amino2 in "${amino_acids[@]}"; do
        ...
        for amino3 in "${amino_acids[@]}"; do
            ...
            for amino4 in "${amino_acids[@]}"; do
                ...
                if [ ! -f "$xtc_file" ]; then
                    echo "no xtc file ${amino1}_${amino2}_${amino3}_${amino4}"
                    continue
                fi
                ...
                if [ ! -f "$log_file" ]; then
                    echo "no log file ${amino1}_${amino2}_${amino3}_${amino4}"
                    continue
                fi
                ...
                output_xvg="out/${amino1}_${amino2}_${amino3}_${amino4}_mon.xvg"
                echo 1 | gmx sasa -f "$xtc_file" -s "$gro_file" -o "$output_xvg" >log/SASA_${amino1}_${amino2}_${amino3}_${amino4}_mon.log 2>&1

                output_xvgi="out/${amino1}_${amino2}_${amino3}_${amino4}_mon_ini.xvg"
                echo 1 | gmx sasa -f "$gro_file" -s "$gro_file" -o "$output_xvgi" >log/SASA_${amino1}_${amino2}_${amino3}_${amino4}_mon_ini.log 2>&1

                # Check if the log file contains the warning
                if grep -q "WARNING: could not" "log/SASA_${amino1}_${amino2}_${amino3}_${amino4}_mon_ini.log"; then
                    echo "Missing VDW radii in ${amino1}_${amino2}_${amino3}_${amino4}: Skipping this loop." >>$missing_vdw_log
                    continue
                fi
