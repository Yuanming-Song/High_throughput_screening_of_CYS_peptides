...
for Cindex in 1 2 3; do
    ...
    output_file="SASA_result_with_common_mon_ini_C${Cindex}.txt"
    >"$output_file" # Clear the output file

    # Search for files matching the pattern #_#_#.xvg
    for file in "$dir"*_*_*.xvg; do
        if [[ $file =~ ([A-Z])_([A-Z])_([A-Z]).xvg ]]; then
            ...
            if [[ -f "$mon_file" ]]; then
                ...
                echo "$letter1 $letter2 $letter3 $ratio_mon_fin $ratio_dim_fin" >>"$output_file"
            fi
        fi
    done
done
