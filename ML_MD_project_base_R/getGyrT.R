getGyrT<-function(dimerized,iniframe=framei) {
  ...
  foreach(frame=iniframe:dim(simtraj$coord)[3]) %dopar% (
    gyrT_largest(frame,AtomIndexLib, cutoff_matrix, AtomIndexList, AtomTypeLib)
  )
