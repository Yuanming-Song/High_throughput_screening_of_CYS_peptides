...
for Cindex in 1 2; do
    outfile="SASA_result_Dipeptide_C$Cindex.dat"
    ...
    maindir="/share/crsp/lab//dtobias/yuanmis1/mrsec/ML-MD-Peptide/MARTINI21/Dipeptide_dis_C${Cindex}/" # Replace with your actual main directory path
    ...
    for amino1 in "${amino_acids[@]}"; do
        ...
        for amino2 in "${amino_acids[@]}"; do
            ...
            if [ ! -f "$log_file" ]; then
                echo "no log file for ${amino1}_${amino2}"
                continue
            fi
            ...
            output_xvg="out/${amino1}_${amino2}.xvg"
            echo 1 | gmx sasa -f "$xtc_file" -s "$gro_file" -o "$output_xvg" >log/SASA_${amino1}_${amino2}.log 2>&1
            output_xvgi="out/${amino1}_${amino2}_dim_ini.xvg"
            echo 1 | gmx sasa -f "$gro_file" -s "$gro_file" -o "$output_xvgi" >log/SASA_${amino1}_${amino2}_dim_ini.log 2>&1 # Check if the log file contains the warning
            if grep -q "WARNING: could not" log/SASA_${amino1}_${amino2}_dim_ini.log; then
                echo "Warning in ${amino1}_${amino2}: Skipping this loop." >>$missing_vdw_log
                continue
            fi
