...
logfile=paste0(outmaindir,"csize_his_anal.log")
...
for (box_dir in box_dirs) {
  ...
  for (seq_dir in seq_dirs) {
    ...
    sizehis<-getdishis_SIRAH(dimerized,TRUE)
    #sizehis<-sizehis/sum(sizehis)
    # Convert matrix to data frame and add additional columns
    sizehis_df <- as.data.frame(cbind(seq(1,length(sizehis),1),sizehis))
    sizehis_df$seq <- seq_name
    sizehis_df$box <- box_size
    
    # Append results to the cumulative data frame
    if (is.null(sum_sizehis)) {
      sum_sizehis <- sizehis_df
    } else {
      sum_sizehis <- rbind(sum_sizehis, sizehis_df)
    }
    
    # Save individual result for this simulation
    out_file <- file.path(outmaindir,"SIRAH_validation_boxsize_csizehis.rda")
    save(sum_sizehis, file = out_file)
  }
}
