...
for amino1 in "${amino_acids[@]}"; do
    ...
    for amino2 in "${amino_acids[@]}"; do
        for amino3 in "${amino_acids[@]}"; do
            ...
            sbatch --job-name="${amino1}_${amino2}_${amino3}_C" \
                ...
                --cpus-per-task=40 \
                --time=4:00:00 \
                --out=out/${amino1}_${amino2}_${amino3}_C.out \
                --wrap="bash //dfs9/tw/yuanmis1/mrsec/ML-MD-Peptide/Tetrapeptide/MARTINI_Setup_script/Cys_unstapled_tetrapeptide_MARTINI_run.sh ${amino1} ${amino2} ${amino3} C"
        done
    done
done
