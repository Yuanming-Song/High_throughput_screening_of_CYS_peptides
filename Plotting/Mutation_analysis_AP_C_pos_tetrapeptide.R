if (readAP) {
  SASA_files <- list()
  # Currently only analyzing C at position 1, but structure allows easy extension
  for (pos in 1:1) {  # Can be changed to 1:4 later
    SASA_files[[pos]] <- read.table(
      paste0("~/Documents/Research/HPC/dfs2/mrsec/ML-MD-Peptide/AP_analysis/Tetrapeptide/SASA_score/SASA_result_with_common_mon_ini_tetra_C", pos, ".txt"),
      header = FALSE,
      col.names = c("Amino1", "Amino2", "Amino3", "Amino4", "ratio_mon_fin", "ratio_dim_fin")
    )
    SASA_files[[pos]]$label <- paste(SASA_files[[pos]]$Amino1, SASA_files[[pos]]$Amino2, 
                                    SASA_files[[pos]]$Amino3, SASA_files[[pos]]$Amino4, sep="_")
  }
}
...
for (pos in 1:length(SASA_files)) {
...
mutation_pairs <- expand.grid(from = residues, to = residues, c_pos = 1:length(SASA_files))
...
plot_df$C_Position <- factor(plot_df$C_Position, 
                           levels = 1:length(SASA_files),
                           labels = paste0("C at position ", 1:length(SASA_files)))
...
mut_detail_plt <- ggplot(plot_df, aes(x = From, y = To, fill = DeltaDeltaAP)) +
  ...
  theme_minimal() +
  labs(title = "Average Change in ΔAP by Mutation (Tetrapeptide)",
       x = "Original Residue",
       y = "Mutated To",
       fill = "ΔΔAP") +
  colored_theme +
...
mut_detail_std_plt <- ggplot(plot_df, aes(x = From, y = To, fill = StdDevAP)) +
  ...
  scale_y_discrete(limits = rev(ordered_residues)) +
  theme_minimal() +
  labs(title = "Standard Deviation of ΔAP by Mutation (Tetrapeptide)",
       x = "Original Residue",
       y = "Mutated To",
       fill = "Std Dev") +
  colored_theme +
...
mut_detail_min_plt <- ggplot(plot_df, aes(x = From, y = To, fill = MinAP)) +
  ...
  scale_y_discrete(limits = rev(ordered_residues)) +
  theme_minimal() +
  labs(title = "Minimum Change in ΔAP by Mutation (Tetrapeptide)",
       x = "Original Residue",
       y = "Mutated To",
       fill = "Min ΔΔAP") +
  colored_theme +
...
mut_detail_max_plt <- ggplot(plot_df, aes(x = From, y = To, fill = MaxAP)) +
  ...
  scale_y_discrete(limits = rev(ordered_residues)) +
  theme_minimal() +
  labs(title = "Maximum Change in ΔAP by Mutation (Tetrapeptide)",
       x = "Original Residue",
       y = "Mutated To",
       fill = "Max ΔΔAP") +
  colored_theme +
...
mut_detail_median_plt <- ggplot(plot_df, aes(x = From, y = To, fill = MedianAP)) +
  ...
  scale_y_discrete(limits = rev(ordered_residues)) +
  theme_minimal() +
  labs(title = "Median Change in ΔAP by Mutation (Tetrapeptide)",
       x = "Original Residue",
       y = "Mutated To",
       fill = "Median ΔΔAP") +
  colored_theme +
...
if (exists("save_plots") && save_plots) {
  ggsave(paste0(APplotDir, "/Tetrapeptide_mutation_AP_C_absolute.png"), 
         plot = mut_detail_plt,
         dpi = 1100, 
         width = 32,
         height = 16,
  ...
  ggsave(paste0(APplotDir, "/Tetrapeptide_mutation_AP_C_std_dev.png"), 
  ...
  ggsave(paste0(APplotDir, "/Tetrapeptide_mutation_AP_C_min.png"), 
  ...
  ggsave(paste0(APplotDir, "/Tetrapeptide_mutation_AP_C_max.png"), 
  ...
  ggsave(paste0(APplotDir, "/Tetrapeptide_mutation_AP_C_median.png"), 
